...
> 按《Hello-Agents》第 1 / 3 / 4 / 7 / 8 / 9 / 12 章的脉络实现，**不依赖任何 Agent 框架，装 4 个包就能跑。**
...
├── main.ipynb               # 教学版 Notebook：按章节顺序点下来就跑完
├── requirements.txt
...
- **智能体范式**：**手写 ReAct**（第 4 章），没有使用任何 Agent 框架
...
因为第 7 章就是教「自己造框架」。先手写一遍，才知道框架替你干了什么。
项目里的模块划分（`llm` / `tools` / `memory` / `context` / `agent` / `evaluate`）和框架的设计是一一对应的。
...
- **章节和文件一一对应，看书能对上代码**：`src/` 下 7 个模块就是 7 个章节的落地，上面有对照表，不用在几千行代码里找「书里那个概念在哪」。
...
- **零框架依赖，`pip install` 4 个包就能跑**：可复现性最好，不会因为某个框架升级就挂掉；想换成 Hello-Agents 框架，改 `src/agent.py` 一个文件就行。
...
感谢 [Datawhale](https://github.com/datawhalechina) 社区的《[Hello-Agents](https://github.com/datawhalechina/hello-agents)》教程
—— 这个项目的每一行代码，都是照着书里第 1、3、4、7、8、9、12 章动手做出来的。
