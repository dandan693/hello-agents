"""
第 3 章「大语言模型基础」→ 落地成一个类：把"跟大模型说话"这件事封装好。

书里的三个要点，这里一一对应：

  3.2.1 提示工程  →  messages 里 system（定人设）/ user（用户说）/ assistant（模型说）
                     三种角色。角色不同，模型的态度就不同。
  3.1.3 Decoder-Only → 一次请求返回一段文本。本文件默认用"非流式"（好解析），
                        另留了 stream() 做打字机效果。
  3.3.2 模型幻觉  →  temperature 默认 0。温度越低，回答越稳定、越可复现，
                     这样评估脚本跑出来的分数才有意义。
